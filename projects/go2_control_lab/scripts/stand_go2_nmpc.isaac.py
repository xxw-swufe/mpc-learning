...
try:
    ...
    from objective_function import BALANCE_WEIGHTS, HEIGHT_STABILIZE_WEIGHTS
    MPC_AVAILABLE = True
    ...
    def solve(self, x0: np.ndarray, reference_dict: dict,
        ...
        try:
            ...
            if res.get("status", "") == "success":
                ...
                # 使用u0进行更稳定的打印（避免U_opt维度问题）
                u0 = np.asarray(res["u0"])  # 第一步控制量，保证是(24,)
                X_opt = np.asarray(res.get("X_opt", []))  # 可选，如果维度有问题不会崩溃
                U_opt = np.asarray(res.get("U_opt", []))

                print("\n=== MPC关键量分析 ===")
                print(f"1. 当前COM速度 x0[3:6]: {x0[3:6]}")
                print(f"2. 参考COM速度 x_ref[0,3:6]: {x_ref[0,3:6]}")
                print(f"3. 四腿足端速度 u0[0:12]:\n{u0[0:12].reshape(4,3)}")
                print(f"4. 四腿GRF u0[12:24]:\n{u0[12:24].reshape(4,3)}")
                print(f"5. 第一步接触 contact_seq[:,0]: {contact_seq[:,0]}")

                # 安全检查X_opt
                if len(X_opt) > 1:
                    print(f"6. 预测下一步COM速度 X_opt[1,3:6]: {X_opt[1,3:6]}")
                else:
                    print("6. X_opt维度异常，无法预测下一步速度")

                # 调试信息
                print(f"   u0 shape: {u0.shape}")
                if len(U_opt) > 0:
                    print(f"   U_opt shape: {U_opt.shape}")
                    print(f"   U_opt[0] shape: {U_opt[0].shape if len(U_opt[0].shape) > 0 else len(U_opt[0])}")
                print("="*50 + "\n")
...
def main():
    ...
    if MPC_AVAILABLE:
        ...
        try:
            ...
            nmpc = CentroidalNMPC_IPOPT_Fixed(
                horizon=N,
                dt=args_cli.dt_mpc,
                weights=HEIGHT_STABILIZE_WEIGHTS  # 使用修复高度维持的权重
            )
    ...
    try:
        ...
        for step in range(max_steps):
            ...
            # MPC控制状态 - 在整个求解周期内保持有效
            # 如果MPC已经接管，默认认为MPC控制有效
            mpc_ok = mpc_takeover_active
            ...
            if is_mpc_tick:
                # 只有在MPC求解时才重置mpc_ok为False，等待新的求解结果
                mpc_ok = False
                if mpc_controller is not None:
                    ...
                    reference_dict = {
                        ...
                        "ref_linear_velocity": v_cmd,  # 使用外部命令速度！
                    ...
                    # 调试打印：确认速度参考正确传入
                    print(f"\n=== 速度参考调试 ===")
                    print(f"外部命令 v_cmd: {v_cmd}")
                    print(f"reference_dict['ref_linear_velocity']: {reference_dict['ref_linear_velocity']}")
                    print(f"期望 x_ref_traj[0,3:6] 应该是: {v_cmd}")
                    print("="*40 + "\n")
                    ...
                    if success:
                        ...
                        # 首次成功接管时打印信息
                        if not mpc_takeover_active:
                            mpc_takeover_active = True
                            print(f"\n=== MPC接管成功！===\n从 t={step*args_cli.dt:.2f}s 开始，MPC完全控制机器人")

                        if step % (mpc_steps_interval * 5) == 0:
                            foot_vel = u[:12].reshape(4, 3)
                            forces = u[12:24].reshape(4, 3)
                            print(f"MPC正常运行中 (成功{mpc_success_count}次) t={step*args_cli.dt:.2f}s")
                            print(f"  足端速度范数: {np.linalg.norm(foot_vel):.3f} m/s")
                            print(f"  总Z轴力: {forces[:, 2].sum():.1f}N")
                    else:
            ...
            if mpc_ok:
                ...
                try:
                    ...
                    # 当MPC成功时，完全使用MPC的输出进行控制
                    actions = q_des_from_mpc
                except Exception as e:
                ...
                # MPC失败时使用固定站姿作为备用
                actions = q_desired
                if mpc_takeover_active:  # 如果之前MPC接管过，现在失败了，才打印
                    print(f"\n⚠️ MPC失败，回退到固定站姿控制\n")
                    mpc_takeover_active = False
